...
__global__ void motion_detection_kernel(const unsigned char* __restrict__ curr_frame,
  ...
  float curr_gray = (curr_frame[rgb_idx] + curr_frame[rgb_idx + 1] + curr_frame[rgb_idx + 2]) / 3.0f;
  float prev_gray = (prev_frame[rgb_idx] + prev_frame[rgb_idx + 1] + prev_frame[rgb_idx + 2]) / 3.0f;
...
void gpu_launch_infer_async(GpuContext& ctx, int idx) {
  ...
  dim3 gridSize((ctx.W + blockSize.x - 1) / blockSize.x, 
                (ctx.H + blockSize.y - 1) / blockSize.y);
  ...
  if (ctx.has_previous_frame[idx]) {
    ...
    motion_detection_kernel<<<gridSize, blockSize, 0, ctx.s1>>>(
      ctx.d_in[idx], ctx.d_prev[idx], ctx.d_out[idx], 
      ctx.W, ctx.H, ctx.motion_threshold
    );
  } else {
    ...
    background_subtraction_kernel<<<gridSize, blockSize, 0, ctx.s1>>>(
      ctx.d_in[idx], ctx.d_background[idx], ctx.d_out[idx],
      ctx.W, ctx.H, ctx.learning_rate, ctx.motion_threshold
    );
  ...
  cuda_check(cudaMemcpyAsync(ctx.h_out[idx], ctx.d_out[idx], pixels * sizeof(float), cudaMemcpyDeviceToHost, ctx.s1), "MemcpyAsync D2H motion");
  cuda_check(cudaMemcpyAsync(ctx.h_stats[idx], ctx.d_stats[idx], stats_blocks * 2 * sizeof(float), cudaMemcpyDeviceToHost, ctx.s1), "MemcpyAsync D2H stats");
  ...
  cuda_check(cudaMemcpyAsync(ctx.d_prev[idx], ctx.d_in[idx], ctx.bytes_per_frame, cudaMemcpyDeviceToDevice, ctx.s1), "MemcpyAsync curr to prev");
}
