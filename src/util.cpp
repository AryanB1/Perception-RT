...
AppConfig load_config(const std::string& path) {
  ...
  if (y["pipeline"]) {
    ...
    if (y["pipeline"]["queue_capacity"]) c.pipeline.queue_capacity = y["pipeline"]["queue_capacity"].as<int>();
    if (y["pipeline"]["drop_policy"])    c.pipeline.drop_policy    = y["pipeline"]["drop_policy"].as<std::string>();
  }
  if (y["controller"]) {
    auto n = y["controller"];
    if (n["switch_up_p95_ms"])    c.deadline.switch_up_p95_ms    = n["switch_up_p95_ms"].as<double>();
    if (n["switch_down_p95_ms"])  c.deadline.switch_down_p95_ms  = n["switch_down_p95_ms"].as<double>();
