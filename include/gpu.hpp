...
inline void gpu_init(GpuContext& ctx, int W, int H, int C = 3, int infer_iters = 2048) {
  ...
  cuda_check(cudaStreamCreateWithPriority(&ctx.s1, cudaStreamNonBlocking, greatestPri),
             "Create stream s1");
  cuda_check(cudaStreamCreateWithPriority(&ctx.s0, cudaStreamNonBlocking, leastPri),
             "Create stream s0");
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    if (ctx.enable_ml_inference) {
      ...
      cuda_check(cudaMalloc(&ctx.d_ml_output[i], ctx.ml_output_size * sizeof(float)),
                 "Malloc d_ml_output");
      cuda_check(cudaHostAlloc(&ctx.h_ml_output[i], ctx.ml_output_size * sizeof(float),
                               cudaHostAllocDefault),
                 "HostAlloc h_ml_output");
    }

    // Allocate pinned host memory
    cuda_check(cudaHostAlloc(&ctx.h_in[i], ctx.bytes_per_frame, cudaHostAllocDefault),
               "HostAlloc h_in");
    cuda_check(cudaHostAlloc(&ctx.h_out[i], pixels * sizeof(float), cudaHostAllocDefault),
               "HostAlloc h_out");
    cuda_check(
        cudaHostAlloc(&ctx.h_stats[i], stats_blocks * 2 * sizeof(float), cudaHostAllocDefault),
        "HostAlloc h_stats");
    ...
    cuda_check(cudaEventCreateWithFlags(&ctx.pre_done[i], cudaEventDisableTiming),
               "Event pre_done");
    cuda_check(cudaEventCreateWithFlags(&ctx.inf_done[i], cudaEventDisableTiming),
               "Event inf_done");
  }
}
